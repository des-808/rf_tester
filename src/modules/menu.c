...
/* Forward declarations for page system */
extern PageDef_t* Page_FindByName(const char* name);
extern PageDef_t* Page_GetRssiPageDef(void);
extern bool Page_OpenFreqAnalyzer(uint32_t freq_mhz, uint32_t bitrate, uint8_t rxbw_index);

/* ========================================================================
 *  Callback-функции для пунктов меню с PAGE/ACTION
 * ======================================================================== */
...
void Menu_ExecuteSelected(UIElement_t* listbox, uint8_t selected_index) 
{
    ...
    switch (item->type) {
        ...
        case ITEM_TYPE_PAGE:
            if (item->data.page_def) {
                listbox->touch_state.drag_last_y = -1;
                listbox->touch_state.drag_active = false;
                
                // Открываем страницу на месте меню
                Page_OpenStatic(item->data.page_def, &digits_node);
                // Блокируем тач на 300мс после перехода
                touch_lock_tick = HAL_GetTick();
            }
            break;
